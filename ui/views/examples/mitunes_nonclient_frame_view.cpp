...
MitunesFrameView::MitunesFrameView()
    : frame_(NULL),
    window_icon_(NULL),
...
void MitunesFrameView::Init(Widget* frame) {
    frame_ = frame;
...
gfx::Rect MitunesFrameView::GetBoundsForClientView() const {
    return client_view_bounds_;
}

gfx::Rect MitunesFrameView::GetWindowBoundsForClientBounds(
...
int MitunesFrameView::NonClientHitTest(const gfx::Point& point) {
    // Sanity check.
...
void MitunesFrameView::GetWindowMask(const gfx::Size& size,
...
void MitunesFrameView::ResetWindowControls() {
...
void MitunesFrameView::UpdateWindowIcon() {
    if (window_icon_)
        window_icon_->SchedulePaint();
}

void MitunesFrameView::UpdateWindowTitle() {
...
void MitunesFrameView::OnPaint(gfx::Canvas* canvas) {
    if (frame_->IsMaximized())
...
void MitunesFrameView::Layout() {
    LayoutClientView();
    LayoutWindowControls();
    LayoutTitleBar();
}

gfx::Size MitunesFrameView::GetPreferredSize() {
    return frame_->non_client_view()->GetWindowBoundsForClientBounds(
        gfx::Rect(frame_->client_view()->GetPreferredSize())).size();
}

gfx::Size MitunesFrameView::GetMinimumSize() {
    return frame_->non_client_view()->GetWindowBoundsForClientBounds(
        gfx::Rect(frame_->client_view()->GetMinimumSize())).size();
}

gfx::Size MitunesFrameView::GetMaximumSize() {
...
void MitunesFrameView::ButtonPressed(Button* sender, const ui::Event& event) {
...
gfx::Rect MitunesFrameView::IconBounds() const {
    int size = IconSize();
...
bool MitunesFrameView::ShouldShowClientEdge() const {
    return !frame_->IsMaximized();
}

void MitunesFrameView::PaintRestoredFrameBorder(gfx::Canvas* canvas) {
...
void MitunesFrameView::PaintMaximizedFrameBorder(gfx::Canvas* canvas) {
    const gfx::ImageSkia* frame_image = GetFrameImage();
    frame_background_->set_theme_image(frame_image);
    frame_background_->set_top_area_height(frame_image->height());
...
void MitunesFrameView::PaintTitleBar(gfx::Canvas* canvas) {
...
void MitunesFrameView::PaintRestoredClientEdge(gfx::Canvas* canvas) {
    return;
...
SkColor MitunesFrameView::GetFrameColor() const {
    return frame_->IsActive() ? kDefaultColorFrame : kDefaultColorFrameInactive;
}

const gfx::ImageSkia* MitunesFrameView::GetFrameImage() const {
#if 0
    return ui::ResourceBundle::GetSharedInstance().GetImageNamed(
        frame_->IsActive() ? IDR_FRAME : IDR_FRAME_INACTIVE).ToImageSkia();
#else
    return ui::ResourceBundle::GetSharedInstance().GetImageNamed(
            frame_->IsActive() ? IDR_MITUNES_BACKGROUND : IDR_MITUNES_BACKGROUND).ToImageSkia();
#endif
}

void MitunesFrameView::LayoutWindowControls() {
...
void MitunesFrameView::LayoutTitleBar() {
...
void MitunesFrameView::LayoutClientView() {
    int top_height = 100;//NonClientTopBorderHeight();
    int border_thickness = NonClientBorderThickness();
...
ImageButton* MitunesFrameView::InitWindowCaptionButton(
